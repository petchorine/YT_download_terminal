...
def get_url_and_verify_integrity():
    try:
        user_url = pyip.inputStr(allowRegexes=[r'https://www.youtube.com'], blockRegexes=".", prompt="Entre ton url :\n >>> ")
    except:
        # TODO : comment paramétrer la réponse dans pyinputplus ???? 
        print("Cette url n'est pas valide.")
    else:
...
def choice_audio_or_video():  
    print("\n")
    try:
        user_choice = pyip.inputMenu(["le(s) clip(s) audio/vidéo", "seulement l'audio"], 
                                prompt="Tu veux télécharger le(s) clip(s) audio/vidéo ou seulement l'audio ?\n",
                                numbered=True)
    except:
        print("écrire une réponse...")
    else: 
        if user_choice == "le(s) clip(s) audio/vidéo":
            print("Tu as choisi de télécharger le (ou les) vidéos.")            
        elif user_choice == "seulement l'audio":
            print("Tu as choisi de télécharger le (ou les) audios.")
        return user_choice


def resolution_choice(choice_media_type, playlist):
    if choice_media_type == "le(s) clip(s) audio/vidéo":
    ...
    elif choice_media_type == "seulement l'audio":
